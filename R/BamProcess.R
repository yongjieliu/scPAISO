...
#'
#' Runs the bundled `bam.split.sh` helper to split paired-end BAMs per chromosome,
#' producing `outpath/chr.bam.{R1,R2}/<sample>/<chr>.bam`.
#'
#' @param bam.r1 Optional character vector of Read1 BAM paths.
#' @param bam.r2 Character vector of Read2 BAM paths; required and must match `samplenames`.
#' @param samplenames Sample identifiers matching the provided BAM vectors.
#' @param outpath Output directory that will receive split BAMs and QC summaries.
#' @param bam.split.sh Optional path to the `bam.split.sh` helper; defaults to `inst/scripts`.
#'
#' @export

BamSplit <- function(bam.r1 = NULL,bam.r2,samplenames,
  outpath = "./scPAISO.output",bam.split.sh = system.file("scripts/bam.split.sh", package = "scPAISO")){
...
PAScalling  <- function(samplenames,chrs,outpath,cell.metadata = NULL,chrom.size,threads = 24,cache.covdata = F,
  ...
  pas_peak_gr <- findPAS(samplenames,chrs,outpath,threads,cache.covdata = F,
                      ...
                      BSg = default_hg38_bsgenome(),literal,num_A)
  return(pas_peak_gr)
}

#' Convert per-chromosome BAMs into annotated data frames
#'
#' @param samplenames Character vector of sample IDs.
#' @param chrs Character vector of chromosome names that were generated by `BamSplit`.
#' @param outpath Directory that stores the `chr.bam.*` folders.
#' @param geneanno GRanges describing gene annotation.
#' @param intronanno GRanges describing intron annotation.
#' @param cell.metadata Optional data frame with columns `cb`, `celltype`, `samplename`.
#' @param chrom.size Data frame holding chromosome lengths (seqnames/seqlengths).
#' @param cache.bam2df Logical; reuse cached bam2df outputs if present.
#' @param threads Number of threads for parallel processing.
#' @param isbulk Logical; set to TRUE for 3'-seq/Poly(A)-seq where only Read2 is used.
#'
#' @export
ProcessBam <- function(samplenames,chrs,outpath,geneanno,intronanno,cell.metadata = NULL,chrom.size,cache.bam2df = FALSE,threads = 24,isbulk = FALSE){
  gr_pas.list <- list()
  combinations <- expand.grid(samplenames, chrs)
